...
function setupAddedLineCount(tabId) {
  const code = `(function getAddedLineCount(){
        ...
        [].forEach.call(document.querySelectorAll('.lines-added'), (e)=>{
          addedLineCount += parseInt(e.textContent, 10 || 0)
        });
...
function setupRemovedLineCount(tabId) {
  const code = `(function getRemovedLineCount(){
        ...
        [].forEach.call(document.querySelectorAll('.lines-removed'), (e)=>{
          removedLineCount += Math.abs(parseInt(e.textContent, 10 || 0))
        });
...
function setupFileNames(tabId) {
  const code = `(function getFileNames(){
        var fileList = '';
        [].forEach.call(document.querySelectorAll('.commit-files-summary--filename'), (e)=>{
...
function setupcommitIDs(tabId) {
  const code = `(function getcommitIDs(){
        var commitList = '';
        var seperator = '..';
        [].forEach.call(document.querySelectorAll('.hash.execute'), (e)=>{
          if (commitList == '') {
            commitList += e.text;
          } else {
